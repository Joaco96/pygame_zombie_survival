...
def create_player(settings,imagen:pygame.Surface = None):
    return create_block(imagen,settings["WIDTH"]//2-settings["PLAYER_WIDTH"],settings["HEIGHT"]//2-settings["PLAYER_HEIGHT"],settings["PLAYER_WIDTH"],settings["PLAYER_HEIGHT"],settings["WHITE"])

def create_zombie(settings,imagen:pygame.Surface = None):
    random_position = generate_random_position(settings)
    return create_block(imagen,random_position[0],random_position[1],settings["ZOMBIE_WIDTH"],settings["ZOMBIE_HEIGHT"],settings["YELLOW"])

def create_bullet(settings,origen:tuple[int,int], target:tuple[int,int],imagen:pygame.Surface = None):
    bullet = create_block(imagen,origen[0]-settings["BULLET_WIDTH"]//2,origen[1]-settings["BULLET_HEIGHT"]//2, settings["BULLET_WIDTH"], settings["BULLET_HEIGHT"],settings["RED"])
